...
function checkGameStatus() {
    if (game.status === "lose") loseInfo.classList.remove("hidden");
    if (game.status === "win") winInfo.classList.remove("hidden");
}
// --ПІДКЛЮЧАЮ стрілки!!!
   ...
   */ restart() {
    ...
    addRandomNumber() {
        ...
        for(let y = 0; y < this.field.length; y++){
            for(let x = 0; x < this.field[y].length; x++)if (this.field[y][x] === 0) arr.push({
                y,
                x
            });
        }
    ...
    transpose(field) {
        ...
        for(let x = 0; x < field.length; x++){
            const newRow = [];
            for(let y = 0; y < field.length; y++)newRow.push(field[y][x]);
    ...
    checkGameOver() {
        ...
        this.same = [];
        for(let y = 0; y < this.field.length; y++){
            for(let x = 0; x < this.field[y].length; x++)if (this.field[y][x] === 0) this.arrZero.push({
                y,
                x
            });
        }
        this.field.forEach((row)=>{
            for(let i = 0; i < row.length; i++)if (row[i] === row[i + 1]) this.same.push(row[i]);
        });
        this.transposeField = this.transpose(this.field);
        this.transposeField.forEach((row)=>{
            for(let i = 0; i < row.length; i++)if (row[i] === row[i + 1]) this.same.push(row[i]);
        });
        if (this.arrZero.length === 0 && this.same.length === 0) this.status = "lose";
    }
    checkWin() {
        // -1-
        // for (let y = 0; y < this.field.length; y++) {
        ...
        if (this.win === true) this.status = "win";
    }
}
module.exports = Game;
